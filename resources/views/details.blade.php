      ...
      <div class="row">
            ...
            <div class="product-single__thumbnail">
              <div class="swiper-container">
                <div class="swiper-wrapper">
                    ...
                    @foreach (explode(",",$product->images) as $gimg)
                    <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto"
                        src="{{asset('uploads/products/thumbnails')}}/{{trim($gimg)}}" width="104" height="104" alt="" /></div>
                    @endforeach
                </div>
              </div>
            </div>
        ...
        <div class="col-lg-5">
          ...
          <div class="product-single__addtolinks">
            ...
            <form method="POST" action="{{route('wishlist.remove',['rowId'=>Cart::instance("wishlist")->content()->Where('id',$product->id)->first()->rowId])}}" id="frm-remove-item">                                    
              @csrf
              @method("DELETE")
              <a href="javascript:void(0)" onclick="document.getElementById('frm-remove-item').submit()" class="menu-link menu-link_us-s add-to-wishlist filled-heart"><svg width="16" height="16" viewBox="0 0 20 20"
                fill="none" xmlns="http://www.w3.org/2000/svg">
                <use href="#icon_heart" />
              </svg><span>Remove from Wishlist</span></a>
              </form>
            @else
              <form method="POST" action="{{route('wishlist.add')}}" id="whishlist-form">
                @csrf
                <input type="hidden" name="id" value="{{$product->id}}" />
                <input type="hidden" name="name" value="{{$product->name}}" />
                <input type="hidden" name="quantity" value="1"/>
                <input type="hidden" name="price" value="{{$product->sale_price == '' ? $product->regular_price:$product->sale_price}}" />
              <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist" onclick="document.getElementById('whishlist-form').submit()"><svg width="16" height="16" viewBox="0 0 20 20"
                fill="none" xmlns="http://www.w3.org/2000/svg">
                <use href="#icon_heart" />
              ...
              <div class="swiper-slide product-card">
                <div class="pc__img-wrapper">
                  <a href="{{ route('shop.product.details',['product_slug'=>$rproduct->slug])}}">
                    ...
                    <img loading="lazy" src="{{asset('uploads/products')}}/{{explode(",",$rproduct->images)[0]}}" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img pc__img-second">
                    @endif
                  </a>
